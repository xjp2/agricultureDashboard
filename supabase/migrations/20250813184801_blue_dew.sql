/*
  ...
  3. Constraints
    - Unique constraint on debtOption.category_name
    - Check constraint on debtData.amount (must be positive)
...
ADD CONSTRAINT debtdata_amount_check CHECK (amount > 0);

-- Add foreign key constraints
ALTER TABLE "debtData"
ADD CONSTRAINT fk_debt_worker
FOREIGN KEY (worker_name) REFERENCES "workersData"("Name") ON DELETE CASCADE;

ALTER TABLE "debtData"
ADD CONSTRAINT fk_debt_category
FOREIGN KEY (category) REFERENCES "debtOption"(category_name) ON DELETE RESTRICT;

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_debt_data_month_year ON "debtData" USING btree (month_year);
CREATE INDEX IF NOT EXISTS idx_debt_data_worker_name ON "debtData" USING btree (worker_name);
CREATE INDEX IF NOT EXISTS idx_debt_data_category ON "debtData" USING btree (category);
CREATE INDEX IF NOT EXISTS idx_debt_option_category_name ON "debtOption" USING btree (category_name);
...
INSERT INTO "debtOption" (category_name, description) VALUES
  ('BOH HING', 'Grocery store debt'),
  ('TONG GAS', 'Gas cylinder debt'),
  ('ROKOK', 'Cigarette debt'),
  ('KEDAI RUNCIT', 'General store debt'),
