...
const WorkOptionManager: React.FC<WorkOptionManagerProps> = ({
  isOpen,
  onClose,
  workOptions,
  onWorkOptionUpdated,
  ...
  const [editingOption, setEditingOption] = useState<WorkOption | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reset form
      setFormData({ work_name: '', description: '' });
      setEditingOption(null);
      onWorkOptionUpdated();
    } catch (err: any) {
      console.error('Error saving work option:', err);
      if (err.code === '23505') {
        setError('Work name already exists');
      } else {
        setError(err.message || 'Failed to save work option');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (option: WorkOption) => {
  ...
  const handleDelete = async (option: WorkOption) => {
    if (!confirm(`Are you sure you want to delete "${option.work_name}"?`)) {
      return;
    }
    ...
    try {
      ...
      console.error('Error deleting work option:', err);
      if (err.code === '23503') {
        setError('Cannot delete work option that is being used in accounting records');
      } else {
        setError(err.message || 'Failed to delete work option');
      }
    }
  };
...
export default WorkOptionManager;